...
mixed do_touch(object me, object item)
{
        ...
        {
                message_vision("$N摸了一下" + item->name() +
                               "，结果手被划破了，痛得大叫一声。\n", me);
        ...
        if (random(1000) == 1 || wizardp(me))
        {
                ...
                message("vision", HIY "你觉得浑身如同笼罩在阳光一般，暖洋洋的，说不出的舒服。\n" NOR, obs);
        } else
        if (me->query("neili") < me->query("max_neili"))
        {
                me->set("neili", me->query("max_neili"));
                tell_object(me, HIY "你觉得一股热气从丹田冉冉升起。\n" NOR);
        }
        return 1;
}
...
mixed weapon_hit_ob(object me, object victim, object weapon, int damage_bonus)
{
        ...
        {
        case 0:
                victim->receive_wound("jing", damage / 4 + random(damage / 4), me);
        ...
        case 1:
                ...
                return HIR "$N" HIR "大喝一声，手中的" + weapon->name() +
                       HIR "遥指$n" HIR "，一道杀气登时将$n" HIR "震退数步。\n" NOR;
